...
class WaliController extends BaseController
{
    ...
    {
        return view('wali/uji', [
            'kriteria' => $this->db->table('kriteria')->get()->getResultArray(),
            'data' => $this->db->table('uji_knn')->where('id_murid', session()->get('id_murid'))->get()->getResultArray()
        ]);
    }

    public function uji_add()
    {
        return view('wali/uji_add', [
            'murid' => $this->db->table('murid')->where('id_murid', session()->get('id_murid'))->get()->getResultArray(),
            'kriteria' => $this->db->table('kriteria')->get()->getResultArray()
        ]);
    }
    ...
    public function uji_save()
    {
        ...
        if (!$this->validate($rules)) {
            return redirect()->to(base_url('WaliPanel/new'))->with('type-status', 'error')->with('dataMessage', $this->validator->getErrors());
        }
        ...
        return redirect()->to(base_url('WaliPanel'))->with('type-status', 'success')->with('message', 'Data berhasil ditambahkan');
    }
    ...
    public function uji_delete($uid)
    {
        ...
        return redirect()->to(base_url('WaliPanel'))->with('type-status', 'success')->with('message', 'Data berhasil dihapus');
    }
    ...
    public function uji_exec()
    {
        ...
        return view('wali/uji_exec', [
            'data' => $data,
            'k' => $this->request->getPost('k'),
            'kriteria' => $this->db->table('kriteria')->get()->getResultArray(),
            'uji' => $this->db->table('uji_knn')->where('id_murid', session()->get('id_murid'))->get()->getResultArray(),
            'dataset' => $dataset
        ]);
    }
}